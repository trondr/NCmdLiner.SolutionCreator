...
namespace NCmdLiner.SolutionCreator.Library.Commands.CreateSolution
{
    public class SolutionCreatorCommandProvider : CommandProvider, ISolutionCreatorCommandProvider
    {
        ...
        public SolutionCreatorCommandProvider(MainWindow mainWindow, IResolveContext resolveContext, IFolderResolver folderResolver, ISolutionTemplateProvider solutionTemplateProvider, ISelectSolutionTemplateWindowFactory selectSolutionTemplateWindowFactory, ILog logger)
        {
        ...
        public int CreateSolution(string targetRootFolder, IConsoleApplicationInfo consoleApplicationInfo)
        {
            ...
            if(selectedTemplate.Name == "Console Application")
            {
        ...
        private static SolutionTemplate GetSelectedSolutionTemplate(SelectSolutionTemplateWindow window)
        {
            if (window.DialogResult != null && window.DialogResult.Value == DialogResult.Ok)
            {
                return Mapper.Map<SolutionTemplate>(window.View.ViewModel.SelectedSolutionTemplate);
            }
            return null;            
        }
    }
